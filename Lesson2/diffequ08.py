...
def heuns_method(num_steps):
    ...
    for step in range(num_steps):
        ...
        x_E = x[step] + h * v[step] # position calculated by Euler's method
        v_E = v[step] + h * initial_acceleration # velocity calculated by Euler's method

        x[step + 1] = x[step] + h * (v[step] + v_E) / 2.
        v[step + 1] = v[step] + h * (initial_acceleration + acceleration(x_E)) / 2.
